...
# ============================================================
#
# 设计核心：格式档案（Format Profile）—— 一个可序列化的 dict，作为格式
# 保持与统一的单一事实来源。两个来源：
#   1) 默认档案：build_default_profile() 由 STYLE_PRESETS 派生
#   2) 参考文档档案：extract_format_profile() 从实际 .docx 采样直接格式
#
# 档案结构（与 STYLE_PRESETS 的 style 配置同构，便于复用 _apply_run_style）：
#   {
#     'page': {'margins': {'top','right','bottom','left'}},   # cm
#     'heading': {1:{...}, 2:{...}, 3:{...}, 4:{...}},
#     'body':   {...}, 'table_header': {...}, 'table_cell': {...},
#     'code': {...}, 'caption': {...}
#   }
# 每个 style 配置可选键：font, east_asia, size_pt, bold, color(hex),
#   alignment('left'/'center'/'right'/'justify'), space_before_pt,
#   space_after_pt, line_spacing_pt, line_spacing_multiple,
#   first_line_indent_pt, first_line_indent_chars, bg_color, indent_cm
# ============================================================
...
def _cfg_from_preset(s):
    """把 STYLE_PRESETS 的 style 配置转为档案 style 配置"""
    cfg = {}
    for k in ('font', 'east_asia', 'size_pt', 'bold', 'color',
              'space_before_pt', 'space_after_pt', 'line_spacing_pt',
              'first_line_indent_chars', 'bg_color', 'border', 'indent_cm'):
        if k in s:
            cfg[k] = s[k]
    if s.get('centered'):
...
def build_default_profile(preset_name='formal_report'):
    """
    由 STYLE_PRESETS 派生默认格式档案。
    preset_name: 'formal_report'/'tech_spec'/'manual'/'checklist'
    """
    preset = STYLE_PRESETS.get(preset_name, STYLE_PRESETS['formal_report'])
    styles = preset.get('styles', {})
    profile = {
        'page': preset.get('page', {'margins': {'top': 2.54, 'right': 3.17,
                                                'bottom': 2.54, 'left': 3.17}}),
        'heading': {},
        'body': _cfg_from_preset(styles.get('Body Text', {})),
        'table_header': _cfg_from_preset(styles.get('Table Header', {})),
        'table_cell': _cfg_from_preset(styles.get('Table Cell', {})),
        'code': _cfg_from_preset(styles.get('Code Block', {})),
        'caption': _cfg_from_preset(styles.get('Caption', {})),
        '_preset': preset_name,
    }
    for lvl in (1, 2, 3, 4):
        key = f'Heading {lvl}'
        if key in styles:
            profile['heading'][lvl] = _cfg_from_preset(styles[key])
    return profile
...
def _detect_heading_level(para):
    """识别段落标题层级：按样式名或 outlineLvl，非标题返回 None"""
    ...
    if pPr is not None:
        ...
        if ol is not None:
            val = ol.get(qn('w:val'))
            try:
                return int(val) + 1  # outlineLvl 0-based
            except (ValueError, TypeError):
                return None
    return None


def _style_chain(para):
    """返回段落的样式继承链（从自身到祖先），用于回退解析有效格式"""
    chain = []
    style = para.style
...
def _effective_run_attr(run, para, attr):
    """run 直接格式优先，缺失则回退样式链"""
    val = getattr(run.font, attr, None)
    if val is not None:
        return val
    for style in _style_chain(para):
...
def _effective_east_asia(run, para):
    """东亚字体：run 直接 rPr 优先，回退样式链 rPr"""
...
def _effective_para_attr(para, attr):
    """段落格式属性：直接优先，回退样式链"""
...
def _snapshot_run_fmt(run, para=None):
    """采样 run 的有效字符格式（直接优先，回退样式）为档案 style 配置"""
    cfg = {}
    if run is None:
        return cfg
    name = run.font.name
    if name is None and para is not None:
        name = _effective_run_attr(run, para, 'name')
    if name:
        cfg['font'] = name
    size = run.font.size
    if size is None and para is not None:
        size = _effective_run_attr(run, para, 'size')
    if size:
        cfg['size_pt'] = size.pt
    bold = run.font.bold
    if bold is None and para is not None:
        bold = _effective_run_attr(run, para, 'bold')
    if bold is not None:
        cfg['bold'] = bold
    ea = None
    rpr = run._element.rPr
    if rpr is not None:
        ...
        if rFonts is not None:
            ea = rFonts.get(qn('w:eastAsia'))
    if not ea and para is not None:
        ea = _effective_east_asia(run, para)
    if ea:
        cfg['east_asia'] = ea
    color = run.font.color.rgb if (run.font.color and run.font.color.type is not None) else None
    if color is None and para is not None:
        try:
            sc = _effective_run_attr(run, para, 'color')
            if sc is not None and sc.type is not None:
                color = sc.rgb
        except Exception:
            color = None
    if color:
...
def _snapshot_para_fmt(para):
    """采样段落级有效格式（含首个 run 的字符格式）为档案 style 配置"""
    cfg = _snapshot_run_fmt(para.runs[0], para) if para.runs else {}
    align = _effective_para_attr(para, 'alignment')
    if align is not None:
        cfg['alignment'] = _ALIGN_MAP.get(align, 'left')
    sb = _effective_para_attr(para, 'space_before')
    if sb is not None:
        cfg['space_before_pt'] = sb.pt
    sa = _effective_para_attr(para, 'space_after')
    if sa is not None:
        cfg['space_after_pt'] = sa.pt
    ls = _effective_para_attr(para, 'line_spacing')
    if ls is not None:
        if isinstance(ls, float):
            cfg['line_spacing_multiple'] = ls
        else:
            cfg['line_spacing_pt'] = ls.pt
    fli = _effective_para_attr(para, 'first_line_indent')
    if fli is not None:
        cfg['first_line_indent_pt'] = fli.pt
    return cfg


def _snapshot_cell_fmt(cell, fallback, force_bold=None):
    """采样表格单元格格式（含背景色），缺失项保留 fallback"""
    cfg = dict(fallback)
    p = cell.paragraphs[0] if cell.paragraphs else None
    if p and p.runs:
        run_cfg = _snapshot_run_fmt(p.runs[0], p)
        cfg.update(run_cfg)
    elif force_bold is not None:
        cfg['bold'] = force_bold
    tcPr = cell._tc.tcPr
    if tcPr is not None:
...
def extract_format_profile(ref_docx_path):
    """
    从参考文档采样实际格式，生成格式档案。
    缺失项回退 formal_report 默认档案。
    """
    ref = Document(ref_docx_path)
    default = build_default_profile('formal_report')
    profile = {
        'page': dict(default['page']),
        'heading': {},
        'body': dict(default['body']),
        'table_header': dict(default['table_header']),
        'table_cell': dict(default['table_cell']),
        'code': dict(default['code']),
        'caption': dict(default['caption']),
        '_source': ref_docx_path,
    }
    ...
    try:
        sec = ref.sections[0]
        m = {}
        for side, attr in (('top', 'top_margin'), ('right', 'right_margin'),
                           ('bottom', 'bottom_margin'), ('left', 'left_margin')):
            val = getattr(sec, attr)
            m[side] = round(val.cm, 2) if val is not None else 2.54
        profile['page'] = {'margins': m}
    except Exception:
        pass

    # 标题 + 正文（采样首个符合条件的段落，与默认档案合并）
    body_sampled = False
    for para in ref.paragraphs:
        if not para.text.strip():
            continue
        lvl = _detect_heading_level(para)
        if lvl:
            if lvl not in profile['heading'] and para.runs:
                cand = _snapshot_para_fmt(para)
                merged = dict(default['heading'].get(lvl, default['body']))
                merged.update(cand)
                profile['heading'][lvl] = merged
        elif not body_sampled and para.runs:
            cand = _snapshot_para_fmt(para)
            # 必须能取到字体+字号才视为有效正文样本；跳过封面级大字号(>15pt)
            sz = cand.get('size_pt')
            if cand.get('font') and sz is not None and sz <= 15:
                merged = dict(default['body'])
                merged.update(cand)
                profile['body'] = merged
                body_sampled = True
    ...
    # 标题层级缺失则用默认补齐
    for lvl in (1, 2, 3, 4):
        if lvl not in profile['heading']:
            profile['heading'][lvl] = dict(default['heading'].get(lvl, default['body']))

    # 表格
    if ref.tables:
        t = ref.tables[0]
        if len(t.rows) >= 1:
            profile['table_header'] = _snapshot_cell_fmt(
                t.rows[0].cells[0], default['table_header'], force_bold=True)
        if len(t.rows) >= 2:
            profile['table_cell'] = _snapshot_cell_fmt(
                t.rows[1].cells[0], default['table_cell'])
...
def save_profile(profile, path):
    ...
    serializable = {k: v for k, v in profile.items() if not k.startswith('_')}
...
def load_profile(path):
    """从 JSON 读取格式档案（heading 键规整为 int）"""
    with open(path, 'r', encoding='utf-8') as f:
        profile = json.load(f)
    # JSON 对象键为字符串，heading 层级需还原为 int 以匹配查找代码
    h = profile.get('heading', {})
    profile['heading'] = {int(k): v for k, v in h.items()}
    return profile


# ---- 档案应用（统一全文格式） ----

def _apply_profile_run(run, cfg):
    """把档案 style 配置应用到 run"""
    if cfg.get('font'):
    ...
    ea = cfg.get('east_asia')
    if ea:
        _set_east_asia(run, ea)
    color = cfg.get('color')
    if color:
        run.font.color.rgb = RGBColor.from_string(color)


def _apply_profile_para_fmt(para, cfg):
    """把档案 style 配置的段落级格式应用到段落"""
    pf = para.paragraph_format
    align = cfg.get('alignment')
    if align and align in _ALIGN_REV:
        para.alignment = _ALIGN_REV[align]
    ...
    elif cfg.get('line_spacing_multiple') is not None:
        pf.line_spacing = cfg['line_spacing_multiple']
    if cfg.get('first_line_indent_pt') is not None:
        pf.first_line_indent = Pt(cfg['first_line_indent_pt'])
    elif cfg.get('first_line_indent_chars') is not None:
        # 1 字符 ≈ 1 个字号磅值；formal_report 正文 12pt → 2 字符=24pt
        size = cfg.get('size_pt', 12)
        pf.first_line_indent = Pt(cfg['first_line_indent_chars'] * size)


def _heading_cfg(profile, level):
    """取标题层级配置，兼容 int/str 键（防手工构造 dict 时踩坑）"""
    h = profile.get('heading', {})
    return h.get(level) or h.get(str(level)) or profile.get('body', {})


def apply_format_profile(doc, profile, apply_page=True, apply_tables=True):
    """
    按档案统一全文格式：重设所有段落（标题/正文）与表格的字符及段落格式。
    不改动文字内容与样式名，仅覆盖直接格式。
    """
    # 页面
    if apply_page:
        margins = profile.get('page', {}).get('margins', {})
        sec = doc.sections[0]
        for side, attr in (('top', 'top_margin'), ('right', 'right_margin'),
                           ('bottom', 'bottom_margin'), ('left', 'left_margin')):
            if margins.get(side) is not None:
                setattr(sec, attr, Cm(margins[side]))

    # 段落（doc.paragraphs 仅含正文级段落，不含表格内段落）
    headings = profile.get('heading', {})
    body_cfg = profile.get('body', {})
    for para in doc.paragraphs:
        lvl = _detect_heading_level(para)
        cfg = _heading_cfg(profile, lvl) if lvl else body_cfg
        for run in para.runs:
            _apply_profile_run(run, cfg)
        _apply_profile_para_fmt(para, cfg)

    # 表格
    if apply_tables:
        for table in doc.tables:
            _apply_profile_table(table, profile)
    return doc


def _apply_profile_table(table, profile):
    """按档案重设表格表头行与数据行的格式"""
    header_cfg = profile.get('table_header', {})
    cell_cfg = profile.get('table_cell', {})
    for ri, row in enumerate(table.rows):
        is_header = (ri == 0)
        cfg = header_cfg if is_header else cell_cfg
        for cell in row.cells:
            ...
            if is_header:
                _set_cell_bg(cell, header_cfg.get('bg_color', 'D5E8F0'))


# ---- 编辑操作（保格式） ----

def _replace_in_para(para, find, replace):
    """
    段落内查找替换，保留格式。
    优先在单个 run 内替换（完整保留段内格式变化）；
    跨 run 的匹配回退为整段重建，套用首个匹配 run 的格式。
    """
    ...
    if find in para.text:
        ...
        for run in para.runs:
            if run.text:
                carrier = run
                break
        fmt = _snapshot_run_fmt(carrier) if carrier else {}
        para_fmt = _snapshot_para_fmt(para)
        new_text = para.text.replace(find, replace)
        count += para.text.count(find)  # 残留匹配数
        clear_runs(para)
        run = para.add_run(new_text)
        _apply_profile_run(run, fmt)
        _apply_profile_para_fmt(para, para_fmt)
    return count


def find_replace_text(doc, find, replace, in_tables=True):
    """
    全文查找替换，保留原格式。返回替换次数。
    覆盖正文段落；in_tables=True 时同时覆盖表格单元格。
    """
    count = 0
    for para in doc.paragraphs:
        count += _replace_in_para(para, find, replace)
    if in_tables:
        for table in doc.tables:
...
def _move_after(ref_element, new_element):
    """把 new_element 移到 ref_element 之后（同一父节点内）"""
    parent = ref_element.getparent()
    parent.insert(list(parent).index(ref_element) + 1, new_element)


def _add_styled_table_profile(doc, headers, rows, profile):
    """按档案创建带样式表格（不依赖 preset_name）"""
    header_cfg = profile.get('table_header', {})
    cell_cfg = profile.get('table_cell', {})
    section = doc.sections[0]
    total_width_emu = section.page_width - section.left_margin - section.right_margin
    ...
    for i, h in enumerate(headers):
        ...
        p = cell.paragraphs[0]
        run = p.add_run(h)
        _apply_run_style(run, header_cfg)
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        _set_cell_bg(cell, header_cfg.get('bg_color', 'D5E8F0'))
    for r, row in enumerate(rows):
        for c, val in enumerate(row):
            ...
            _apply_run_style(run, cell_cfg)
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    return table


def _add_code_block_profile(doc, text, profile):
    """按档案创建代码块段落"""
    code_cfg = profile.get('code', {})
    p = doc.add_paragraph()
    run = p.add_run(text)
    run.font.name = code_cfg.get('font', 'Consolas')
    run.font.size = Pt(code_cfg.get('size_pt', 9))
    _set_east_asia(run, code_cfg.get('east_asia', '宋体'))
    pPr = p._element.get_or_add_pPr()
    shd = parse_xml(f'<w:shd {nsdecls("w")} w:fill="{code_cfg.get("bg_color", "F2F2F2")}" w:val="clear"/>')
    pPr.append(shd)
    p.paragraph_format.left_indent = Cm(code_cfg.get('indent_cm', 1))
    if code_cfg.get('line_spacing_pt'):
        p.paragraph_format.line_spacing = Pt(code_cfg['line_spacing_pt'])
    return p


def insert_blocks_after(doc, anchor_text, blocks, profile):
    """
    在首个包含 anchor_text 的段落之后，依次插入带类型的内容块，每块按档案格式化。
    blocks: list of dict，支持的 type：
        {'type':'heading','level':2,'text':'...'}
        {'type':'body','text':'...'}
        {'type':'caption','text':'...'}
        {'type':'code','text':'...'}
        {'type':'table','headers':[...],'rows':[[...],...]}
    返回锚点段落索引；找不到锚点抛 ValueError。
    """
    ...
    for block in blocks:
        ...
        if btype == 'heading':
            ...
            try:
                ...
                pass
            run = new_para.add_run(text)
            cfg = _heading_cfg(profile, level)
            _apply_profile_run(run, cfg)
            _apply_profile_para_fmt(new_para, cfg)
            _move_after(insert_after, new_para._element)
            insert_after = new_para._element
        elif btype in ('body', 'caption'):
            new_para = doc.add_paragraph()
            run = new_para.add_run(text)
            cfg = profile.get(btype, profile.get('body', {}))
            _apply_profile_run(run, cfg)
            _apply_profile_para_fmt(new_para, cfg)
            _move_after(insert_after, new_para._element)
            insert_after = new_para._element
        elif btype == 'code':
            new_para = _add_code_block_profile(doc, text, profile)
            _move_after(insert_after, new_para._element)
            insert_after = new_para._element
        elif btype == 'table':
            table = _add_styled_table_profile(
                doc, block.get('headers', []), block.get('rows', []), profile)
            if table is not None:
                _move_after(insert_after, table._tbl)
                insert_after = table._tbl
    return idx


def set_heading(doc, para_idx, level, profile):
    """
    把已有段落重设为 N 级标题：改样式 + 套档案标题格式。用于重排标题。
    """
...
def refresh_toc(doc):
    """
    标记文档的 TOC 域为脏，使 Word 打开时自动重建目录。
    """
    settings = doc.settings.element
    upd = settings.find(qn('w:updateFields'))
    if upd is None:
        upd = parse_xml(f'<w:updateFields {nsdecls("w")} w:val="true"/>')
        settings.append(upd)
    else:
...
def ensure_toc(doc, title='目  录'):
    """
    若文档无 TOC 域则在末尾插入一个；返回是否新增。
    无论新增与否，都标记 TOC 为待更新。
    """
    body = doc.element.body
    has_toc = False
    for el in body.iter():
        if el.tag == qn('w:instrText') and el.text and 'TOC' in el.text:
            has_toc = True
            break
    if not has_toc:
        add_toc(doc, title)
...
def edit_doc(doc_or_path):
    """加载目标文档用于编辑（路径或 Document 对象均接受）"""
